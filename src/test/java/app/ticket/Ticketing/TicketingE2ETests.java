...
public class TicketingE2ETests {
    ...
    void checkTicketSuccessTest() {
        ...
        RestAssured.given().contentType(ContentType.JSON).when().get("/ticket/" + testParam.getTicketId()).then()
                        .statusCode(200);
    }
    ...
    void createTicketSuccessTest() {
        ...
        RestAssured.given().contentType(ContentType.JSON).body(testParam).when().post("/ticket").then().statusCode(201);
    }
    ...
    void createTicketFailTest1() {
        TicketingRequestDto testParam = setData();
        RestAssured.given().contentType(ContentType.JSON).body(testParam).when().post("/ticket").then().statusCode(409)
                        .and().body("message", is("이미 예약된 공연입니다."));
    }
    ...
    void createTicketFailTest2() {
        ...
        RestAssured.given().contentType(ContentType.JSON).body(testParam).when().post("/ticket").then().statusCode(403)
